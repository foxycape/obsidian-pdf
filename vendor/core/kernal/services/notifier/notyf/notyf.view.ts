...
export class NotyfView {
  ...
  constructor(public rootElement: Document | HTMLElement) {
    // Creates the main notifications container
    const docFrag = rootElement.ownerDocument ? rootElement.ownerDocument.createDocumentFragment() : (rootElement as Document).createDocumentFragment();
    ...
    if (rootElement.ownerDocument) {
      ...
      (rootElement as Document).body.appendChild(docFrag);
    }
  ...
  public on(event: NotyfEvent, cb: NotyfEventCallback) {
    ...
    let r = { [event]: cb };
    this.events = Object.assign(this.events, r);
  }
  ...
  private _createHTMLElement({
    ...
    const elem = this.rootElement.ownerDocument ? this.rootElement.ownerDocument.createElement(tagName) : (this.rootElement as Document).createElement(tagName);
    if (className) {
  ...
  private _createA11yContainer() {
    ...
    if (this.rootElement.ownerDocument) {
      this.rootElement.appendChild(a11yContainer);
    }
    else {
      (this.rootElement as Document).body.appendChild(a11yContainer);
    }
  ...
  private _getAnimationEndEventName(): string {
    const el = this.rootElement.ownerDocument ? this.rootElement.ownerDocument.createElement('_fake') : (this.rootElement as Document).createElement('_fake');
