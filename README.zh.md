...
社区安装只会拿到 Obsidian 标准三文件。Worker 与 signer 在首次加载时从 `main.js` 解压（无需下载）。字符映射与标准字体（约 1.5 MB）在缺失时从 `foxycape-pdf-assets.zip` 静默下载，按 **PDF.js 版本** 识别，因此日常插件升级不会再次拉取。本地 `npm run link` 已带 sidecar，无需下载。
