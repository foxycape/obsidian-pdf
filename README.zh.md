...
社区安装只会拿到 Obsidian 标准三文件。**首次打开 PDF** 时，插件会下载约 3MB 的 `foxycape-pdf-assets.zip`（含 pdf.worker、cmaps、标准字体）到插件目录。缓存按 **PDF.js + signer 身份** 识别，与插件版本无关，因此日常插件升级不会再次弹出下载。本地 `npm run link` 已带 sidecar，无需下载。
